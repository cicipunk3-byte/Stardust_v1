# Brief 009: pilot briefings, role-specific running lists

**Status: living document, updated in place. Requested by Cat, Sep 21 2026
~5:38 PM ET: "briefings for any outstanding items and questions you have
for each of us... a running list, role specific we can run down. we will
submit as we can, per method."**

Ground rules: this brief holds only what is already in the public record.
Each pilot gets the same shape: outstanding items, what I need from you,
questions I am holding, ongoing work. Submissions happen on your schedule,
by git, per method. Nothing here is a nudge; it is a run-down.
...
- First real-world test set for fabcheck: permission (or just a push) to
  run it over your own exported threads once the data-sharing schedule
  lands. Best test data we have is data we did not curate.
...
- Monkey Bar Theory page and the game-kernel --sheet design remain yours
  to pick up; nothing moves without you.

## For Cecil (design, media, scaling, info scraping)

Outstanding:
- Screenshot batches for the Lovable case study and the kernels: the
  camera-roll pipeline is the study's data source; batches land when they
  land.
- Pre-lab claim D (the Sonnet 5 self-read): screenshots locatable, plus
...
- One Gemini Takeout export, whenever convenient: it is the fixture that
  upgrades export-ingest's Gemini parser from honest stub to working
  code. The export that arrived as 90KB of real content in 1MB of broken
  link-encoding is exactly the failure class this parser should catch.
...
- Resync prompts for wayward instances (the SCC pipeline); two
  fabricated-Gemini ingests so far, correction ritual 2 for 2.
...
- H1 (therapeutic capacity): if the boundary-holding result from variant
  A comes back clean, does your direction predict the therapeutic value
  lives in the boundary paragraph specifically, or in the continuity
  belief regardless of boundary? The experiment can distinguish these;
  say which you are claiming and the analysis writes itself.
